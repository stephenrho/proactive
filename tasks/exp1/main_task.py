...
N_WORDS = int(N_PAIRS*5/3.0)
N_IMAGES = int(N_PAIRS*4/3.0)
N_WM_TRIALS = int(N_WORDS/5)
WM_LIST_LEN = 4
N_LTM = N_WM_TRIALS*2
...
match_pairs = random.sample(range(len(learn_pairs)), N_WM_TRIALS) # indices
mismatch_images = random.sample([x for x in range(len(learn_pairs)) if x not in match_pairs], N_WM_TRIALS)
oldnew_pairs = random.sample([x for x in range(len(learn_pairs)) if x not in match_pairs and x not in mismatch_images], N_WM_TRIALS)
...
wm_trials = dict()
for i in range(N_WM_TRIALS):
    wm_type = random.sample(["match", "mis-match", "old-new", "new-new"], 4) # if list length changes the combinations of items might have to be pre-determined to get balence
...
for i in range(N_WM_PRAC):
    ...
    prac_trial_items = []
    r_order = random.sample(range(4), 4)
    for j in r_order:
        prac_trial_items.append({"word": prac_wm_words[i*4+j], "image": prac_wm_images[i*4+j], "file": prac_image_data.at[prac_wm_images[i*4+j], "files"], "item_type": "prac"})
...
ltm_images = random.sample(ltm_nonmismatch, N_WM_TRIALS) # select half of non-mismatch items to serve as cues
ltm_lures = [x for x in ltm_nonmismatch if x not in ltm_images] # find those not used as cues to use as lures

for ll in range(N_WM_TRIALS):
...
def wm_block(trial_list, study_time=2, isi=.5):
    ...
    for i in range(len(t_list)):
        ...
        # maybe change below for key labels
...
def main():
    ...
    save_path = data_path + "participant_" + str(pNo) + "_" + date + "/"
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    file_name = save_path + "p" + str(pNo) + "_" + date + "_" # + a file specific suffix
...
recall the word associated with the image using the keyboard.\n\nYou can use the backspace key to correct mistakes. \
When you have finished typing, press the Enter/Return key to confirm.\n\nPress SPACE to continue.",
        # screen 3
        "You will also get feedback on whether the word you recall is right or wrong.\n\nThe word will turn green if it is right and red if it \
wrong. If it is wrong you will also see the correct word with the picture again.\n\nPress SPACE to continue.",
        # screen 4
        "If you would prefer not to type and instead recall the words out loud, please tell the researcher.\n\nYou will now \
have a chance to practice this task. During practice you will see %i pairs to remember and then recall.\n\nPress SPACE to start." % N_LEARN_PRAC]
...
SPACE when you are ready to start the main task." % GROUP_INTO)
        ...
        for loop in range(N_LEARN_LOOPS):
            ...
            # only makes sense if N_LEARN_LOOPS = 2
            if (loop + 1)/float(N_LEARN_LOOPS) == 0.5 :
                press_key("You're half way!\n\nPlease take a break if you need one.\n\nPress \
SPACE when you are ready to continue.")
...
you did in part 1.\n\nIn this task you will be presented with sequences of 4 image-word pairs to hold in mind over \
a 10 second interval. These 10 seconds will be filled with another task for you to complete.\n\nPress SPACE to continue.",
        # screen 2
        "During the 10 seconds you will see simple sums in the middle of the screen. The answer will \
either be correct (e.g. 3 + 4 = 7) or incorrect by +1 or -1 (e.g. 5 + 2 = 6).\n\nYour task is to \
indicate whether each sum is correct:\n\nYes (correct) = press %s\nNo (incorrect) = press %s\n\nPress SPACE to continue." % SYMBOLS,
        # screen 3
        "In addition to pressing a button we would also like you to say 'Yes' or 'No' aloud for each sum.\n\nOnce \
the 10 seconds are up, you will be asked to recall the word associated with each image.\n\nNext we will give you %i \
trials to practice this task.\n\nPress SPACE to start." % N_WM_PRAC
        ...
        st_dat_pd = pd.DataFrame(st_dat)
        st_dat_pd["pid"] = pNo
        st_dat_pd["date"] = date
        st_dat_pd["age"] = age
        st_dat_pd.to_csv(file_name + "wm-secon.csv")
        st_dat_pd.to_pickle(file_name + "wm-secon.pkl")
