...
let expName = 'proactive';  // from the Builder filename that created this script
...
function updateInfo() {
  ...
  psychoJS.setRedirectUrls('https://app.prolific.co/submissions/complete?cc=45A2BC42', '');
...
function experimentInit() {
  ...
  RIClock = new util.Clock();
  ri_blank = new visual.TextStim({
    win: psychoJS.window,
    name: 'ri_blank',
    text: '',
    font: 'Arial',
...
function prac_wm_trial_loopLoopBegin(thisScheduler) {
  ...
  prac_wm_trial_loop.forEach(function() {
    ...
    thisScheduler.add(RIRoutineBegin(snapshot));
    thisScheduler.add(RIRoutineEachFrame(snapshot));
    thisScheduler.add(RIRoutineEnd(snapshot));
...
function STRoutineEachFrame(trials) {
  return function () {
    ...
    if (st_key.status === PsychoJS.Status.STARTED) {
      let theseKeys = st_key.getKeys({keyList: ['space'], waitRelease: false});
...
var RIComponents;
function RIRoutineBegin(trials) {
  return function () {
    ...
    // keep track of which components have finished
    RIComponents = [];
    RIComponents.push(ri_blank);
    
    RIComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function RIRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'RI'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    t = RIClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    ...
    // *ri_blank* updates
    if (t >= 0.0 && ri_blank.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ri_blank.tStart = t;  // (not accounting for frame time here)
    ...
    frameRemains = 0.0 + 2 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (ri_blank.status === PsychoJS.Status.STARTED && t >= frameRemains) {
