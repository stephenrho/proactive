...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/stephenrhodes/Dropbox/Current Projects/Baycrest/proactive/online/exp3/task-dist/proactive3.1_lastrun.py',
...
wm_text = visual.TextStim(win=win, name='wm_text',
    text='The task in this section also uses pairs of images and words but is different from the task you did in section 1. In this section we are looking at short-term memory for pairs.\n\nIn this task you will be presented with sequences of 4 image-word pairs to hold in mind over a 10 second interval. During this 10 second interval you will be asked to perform another task (more on this soon).\n\nPress SPACE to continue.\n',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=1.6, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
text_6 = visual.TextStim(win=win, name='text_6',
    text="After the 10 second interval you will be asked to recall the word associated with each image by typing it on the keyboard then pressing enter (or return) to submit. There won't be any feedback in this section.\n\nNext we will tell you more about the task your are asked to do during the 10 second interval.\n\nPress SPACE to start.",
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=1.6, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
top_text = visual.TextStim(win=win, name='top_text',
    text='During the 10 second interval between seeing the pairs you need to remember and recalling them there is a search task. In this task you will see square symbols with open sides. All but one of these symbols will have an open side at the top or bottom.',
    font='Arial',
...
text_14 = visual.TextStim(win=win, name='text_14',
    text='Well done!\n\nNow we will practice both tasks together. Remember, you will see 4 image-word pairs to remember. Then during the 10 second interval you will have 4 search problems to respond to where you find the target square and indicate if it is open on the left or right. After that you will be asked to type the word associated with each image you studied.\n\nThere will be 3 example sequences to practice the tasks together.\n\nPress SPACE to continue.',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=1.6, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
# Initialize components for Routine "search_miss"
search_missClock = core.Clock()
...
for thisPrac_wm_trial_loop in prac_wm_trial_loop:
    ...
    # set up handler to look after randomisation of conditions etc
    prac_wm_search_loop = data.TrialHandler(nReps=4, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='prac_wm_search_loop')
    thisExp.addLoop(prac_wm_search_loop)  # add the loop to the experiment
    thisPrac_wm_search_loop = prac_wm_search_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisPrac_wm_search_loop.rgb)
    if thisPrac_wm_search_loop != None:
        for paramName in thisPrac_wm_search_loop:
            exec('{} = thisPrac_wm_search_loop[paramName]'.format(paramName))
    
    for thisPrac_wm_search_loop in prac_wm_search_loop:
        currentLoop = prac_wm_search_loop
        # abbreviate parameter names if possible (e.g. rgb = thisPrac_wm_search_loop.rgb)
        if thisPrac_wm_search_loop != None:
            for paramName in thisPrac_wm_search_loop:
                exec('{} = thisPrac_wm_search_loop[paramName]'.format(paramName))
        ...
        for thisComponent in search_isiComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        prac_wm_search_loop.addData('text_9.started', text_9.tStartRefresh)
        prac_wm_search_loop.addData('text_9.stopped', text_9.tStopRefresh)
        ...
        for thisComponent in searchComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        prac_wm_search_loop.addData('sim1.started', sim1.tStartRefresh)
        prac_wm_search_loop.addData('sim1.stopped', sim1.tStopRefresh)
        prac_wm_search_loop.addData('sim2.started', sim2.tStartRefresh)
        prac_wm_search_loop.addData('sim2.stopped', sim2.tStopRefresh)
        prac_wm_search_loop.addData('sim3.started', sim3.tStartRefresh)
        prac_wm_search_loop.addData('sim3.stopped', sim3.tStopRefresh)
        prac_wm_search_loop.addData('sim4.started', sim4.tStartRefresh)
        prac_wm_search_loop.addData('sim4.stopped', sim4.tStopRefresh)
        prac_wm_search_loop.addData('sim5.started', sim5.tStartRefresh)
        prac_wm_search_loop.addData('sim5.stopped', sim5.tStopRefresh)
        prac_wm_search_loop.addData('sim6.started', sim6.tStartRefresh)
        prac_wm_search_loop.addData('sim6.stopped', sim6.tStopRefresh)
        prac_wm_search_loop.addData('sim7.started', sim7.tStartRefresh)
        prac_wm_search_loop.addData('sim7.stopped', sim7.tStopRefresh)
        prac_wm_search_loop.addData('sim8.started', sim8.tStartRefresh)
        prac_wm_search_loop.addData('sim8.stopped', sim8.tStopRefresh)
        # check responses
        if search_resp.keys in ['', [], None]:  # No response was made
            search_resp.keys = None
        prac_wm_search_loop.addData('search_resp.keys',search_resp.keys)
        if search_resp.keys != None:  # we had a response
            prac_wm_search_loop.addData('search_resp.rt', search_resp.rt)
        prac_wm_search_loop.addData('search_resp.started', search_resp.tStartRefresh)
        prac_wm_search_loop.addData('search_resp.stopped', search_resp.tStopRefresh)
        prac_wm_search_loop.addData('search_fix.started', search_fix.tStartRefresh)
        prac_wm_search_loop.addData('search_fix.stopped', search_fix.tStopRefresh)
        
        # ------Prepare to start Routine "search_miss"-------
        continueRoutine = True
        # update component parameters for each repeat
        if search_resp.keys not in ["0", "1"]:
            miss_count += 1
        # keep track of which components have finished
        search_missComponents = []
        for thisComponent in search_missComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        search_missClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "search_miss"-------
        while continueRoutine:
            # get current time
            t = search_missClock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=search_missClock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
        ...
        # -------Ending Routine "search_miss"-------
        for thisComponent in search_missComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # the Routine "search_miss" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 4 repeats of 'prac_wm_search_loop'
...
for thisWm_trial_loop in wm_trial_loop:
    ...
    # set up handler to look after randomisation of conditions etc
    wm_search_loop = data.TrialHandler(nReps=4, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='wm_search_loop')
    thisExp.addLoop(wm_search_loop)  # add the loop to the experiment
    thisWm_search_loop = wm_search_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisWm_search_loop.rgb)
    if thisWm_search_loop != None:
        for paramName in thisWm_search_loop:
            exec('{} = thisWm_search_loop[paramName]'.format(paramName))
    
    for thisWm_search_loop in wm_search_loop:
        currentLoop = wm_search_loop
        # abbreviate parameter names if possible (e.g. rgb = thisWm_search_loop.rgb)
        if thisWm_search_loop != None:
            for paramName in thisWm_search_loop:
                exec('{} = thisWm_search_loop[paramName]'.format(paramName))
        ...
        for thisComponent in search_isiComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        wm_search_loop.addData('text_9.started', text_9.tStartRefresh)
        wm_search_loop.addData('text_9.stopped', text_9.tStopRefresh)
        ...
        for thisComponent in searchComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        wm_search_loop.addData('sim1.started', sim1.tStartRefresh)
        wm_search_loop.addData('sim1.stopped', sim1.tStopRefresh)
        wm_search_loop.addData('sim2.started', sim2.tStartRefresh)
        wm_search_loop.addData('sim2.stopped', sim2.tStopRefresh)
        wm_search_loop.addData('sim3.started', sim3.tStartRefresh)
        wm_search_loop.addData('sim3.stopped', sim3.tStopRefresh)
        wm_search_loop.addData('sim4.started', sim4.tStartRefresh)
        wm_search_loop.addData('sim4.stopped', sim4.tStopRefresh)
        wm_search_loop.addData('sim5.started', sim5.tStartRefresh)
        wm_search_loop.addData('sim5.stopped', sim5.tStopRefresh)
        wm_search_loop.addData('sim6.started', sim6.tStartRefresh)
        wm_search_loop.addData('sim6.stopped', sim6.tStopRefresh)
        wm_search_loop.addData('sim7.started', sim7.tStartRefresh)
        wm_search_loop.addData('sim7.stopped', sim7.tStopRefresh)
        wm_search_loop.addData('sim8.started', sim8.tStartRefresh)
        wm_search_loop.addData('sim8.stopped', sim8.tStopRefresh)
        # check responses
        if search_resp.keys in ['', [], None]:  # No response was made
            search_resp.keys = None
        wm_search_loop.addData('search_resp.keys',search_resp.keys)
        if search_resp.keys != None:  # we had a response
            wm_search_loop.addData('search_resp.rt', search_resp.rt)
        wm_search_loop.addData('search_resp.started', search_resp.tStartRefresh)
        wm_search_loop.addData('search_resp.stopped', search_resp.tStopRefresh)
        wm_search_loop.addData('search_fix.started', search_fix.tStartRefresh)
        wm_search_loop.addData('search_fix.stopped', search_fix.tStopRefresh)
    ...
    # completed 4 repeats of 'wm_search_loop'
