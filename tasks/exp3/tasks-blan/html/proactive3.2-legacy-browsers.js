...
let expName = 'proactive3.2';  // from the Builder filename that created this script
...
function updateInfo() {
  ...
  psychoJS.setRedirectUrls('https://app.prolific.co/submissions/complete?cc=1BCC9D71', '');
...
function experimentInit() {
  ...
  text_14 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_14',
    text: 'We will now practice this task. Remember, you will see 4 image-word pairs to remember. After a 10 second interval you will be asked to type the word associated with each image you studied.\n\nThere will be 3 example sequences to practice.\n\nPress SPACE to continue.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], height: 0.1,  wrapWidth: 1.6, ori: 0,
    color: new util.Color('black'),  opacity: 1,
  ...
  // Initialize components for Routine "wm_ri"
  wm_riClock = new util.Clock();
...
function prac_wm_trial_loopLoopBegin(thisScheduler) {
  ...
  prac_wm_trial_loop.forEach(function() {
    ...
    thisScheduler.add(wm_riRoutineBegin(snapshot));
    thisScheduler.add(wm_riRoutineEachFrame(snapshot));
    thisScheduler.add(wm_riRoutineEnd(snapshot));
...
var wm_riComponents;
function wm_riRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'wm_ri'-------
    t = 0;
    wm_riClock.reset(); // clock
    frameN = -1;
    routineTimer.add(10.000000);
    // update component parameters for each repeat
    // keep track of which components have finished
    wm_riComponents = [];
    wm_riComponents.push(text_9);
    
    wm_riComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function wm_riRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'wm_ri'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    t = wm_riClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    if ((prac !== 1)) {
        if ((list_no !== expInfo["list"])) {
...
function wm_riRoutineEnd(trials) {
  return function () {
    //------Ending Routine 'wm_ri'-------
    wm_riComponents.forEach( function(thisComponent) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
