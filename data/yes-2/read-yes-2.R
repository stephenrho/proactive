...
# read data from the 2 s interval, with distraction condition
...
for (i in 1:length(groups)){
  ...
  for (x in files){
    ...
    if (!(nrow(tmp)%in% c(966, 1326, 1686))){
      cat(groups[i], ": file ", x, " is incomplete\n")
      cat(nrow(tmp), " rows\n")
    } else if (ncol(tmp) != 138){
      cat(groups[i], ": file ", x, " is incomplete\n")
      cat(ncol(tmp), " cols\n")
    } else {
...
search_dat$wm_trial[search_dat$section=="wm"] = 1:19
...
# write data to csv
write.csv(x = wm_dat, file = "yes2_wm.csv", row.names = F)
write.csv(x = learn_dat, file = "yes2_learn.csv", row.names = F)
write.csv(x = search_dat, file = "yes2_search.csv", row.names = F)
