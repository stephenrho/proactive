...
# read data from the 10 s interval, no distraction condition
...
prolific_data = prolific_files[c(grep(pattern = "younger-blan", x = prolific_files),
                                 grep(pattern = "older-blan", x = prolific_files))]
...
for (i in 1:length(groups)){
  ...
  for (x in files){
    ...
    if (!(nrow(tmp)%in% c(946, 1306, 1666)) | ncol(tmp) != 114){
...
wm_dat = reshape(full_wm_dat[,c("participant", "group", "trial_no", unlist(col_list))], 
...
# write data to csv
write.csv(x = wm_dat, file = "no10_wm.csv", row.names = F)
write.csv(x = learn_dat, file = "no10_learn.csv", row.names = F)
